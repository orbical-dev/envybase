...
# Google setup
if "google" in config.SOCIAL_LOGINS and hasattr(config, 'GOOGLE_CLIENT_ID') and hasattr(config, 'GOOGLE_CLIENT_SECRET'):
    providers['google'] = {
...
# GitHub setup
if "github" in config.SOCIAL_LOGINS and hasattr(config, 'GITHUB_CLIENT_ID') and hasattr(config, 'GITHUB_CLIENT_SECRET'):
    providers['github'] = {
...
async def log_and_raise(exc: Exception, error_type: str, envy_code: str, error_id: int, http_status: int = 400):
    ...
    try:
        if provider == 'github':
            ...
            async with httpx.AsyncClient() as c:
                ...
                user_resp = await c.get(providers['github']['userinfo_endpoint'], headers=headers)
                data = user_resp.json()
                # Email fallback
                if not data.get('email'):
                    emails_resp = await c.get(providers['github']['email_endpoint'], headers=headers)
                    primary = next((e['email'] for e in emails_resp.json() if e.get('primary') and e.get('verified')), None)
                    data['email'] = primary
                # Map GitHub fields
                data['given_name'] = data.get('name', '').split(' ')[0] if data.get('name') else ''
                data['family_name'] = ' '.join(data.get('name', '').split(' ')[1:]) if data.get('name') else ''
...
async def oauth2_callback(request: Request, provider: str):
    ...
    if not email:
        await log_and_raise(Exception('Email missing'), 'MissingEmailError', '300x4', error_id)
    ...
    if user:
        ...
        if user.get('provider') != provider:
            await log_and_raise(
                Exception(f"User registered with {user.get('provider')}, attempted {provider}"),
